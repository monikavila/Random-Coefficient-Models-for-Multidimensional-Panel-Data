...
x_1=0*ones(N1*N2*T,1);                      % Auxiliary column that allows the loop to start at 2 
X_1=[x_1 X];                              % Auxiliary matrix that allows the loop to start at 2 
H_alpha1K=0*ones(N1*N2*T,(floor(K/2)+1)*N1*N2*T);    % Matrix for storage the H matrices  
H_gamma1K=0*ones(N1*N2*T,(floor(K/2)+1)*N1*N2*T);    % Matrix for storage the H matrices  
H_lambda1K=0*ones(N1*N2*T,(floor(K/2)+1)*N1*N2*T);   % Matrix for storage the H matrices  
...
% then we do from 1 to floor(K/2), and after from floor(K/2)+1 to K 
    ...
    a=diag(X_1(:,i+1)); % obtain the column that we want to diagonalize to have X_k
